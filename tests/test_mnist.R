...
fit = XBART.multinomial(y=matrix(y), num_class=10, X=X_train, Xtest=X_test, 
                        ...
                        no_split_penality = 1, weight = seq(1, 10, 0.5), burnin = burnin, mtry = mtry, p_categorical = p, 
                        kap = 1, s = 1, verbose = TRUE, parallel = TRUE, set_random_seed = TRUE, 
                        random_seed = NULL, sample_weights_flag = TRUE, stop_threshold = 0.1, nthread = 0) 
t = proc.time() - t
...
# }
# 
saveRDS(fit, paste(path, 'mnist_result/mnist_weight_per_class.rds', sep = ''))
