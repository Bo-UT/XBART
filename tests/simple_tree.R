...
get_XBART_params <- function(n, d, y) {
  XBART_params = list(num_trees = 2,                 # number of trees 
                      ...
                      mtry = 2,               # number of variables sampled in each split
                      burnin = 2,
                      no_split_penality = "Auto"
  )            # burnin of MCMC sample
  num_tress = XBART_params$num_trees
  XBART_params$max_depth = matrix(250, num_tress, XBART_params$num_sweeps)
  XBART_params$num_cutpoints = 5;                                           # number of adaptive cutpoints
...
dcat = 1 # number of categorical variables
# must be d >= dcat
...
if (new_data) {
  if (d != dcat) {
    x = matrix(runif((d - dcat) * n, -1, 1), n, d - dcat)
    if (dcat > 0) {
      x = cbind(x, matrix(as.numeric(sample(0:1, dcat * n, replace = TRUE)), n, dcat))
    }
  } else {
  ...
  if (d != dcat) {
    xtest = matrix(runif((d - dcat) * nt, -1,1), nt, d - dcat)
    if (dcat > 0) {
      xtest = cbind(xtest, matrix(as.numeric(sample(0:1, dcat * nt, replace = TRUE)), nt, dcat))
    }
  } else {
    xtest = matrix(as.numeric(sample(0:1, cat * nt, replace = TRUE)), nt, dcat)
  }
...
fit = XBART(as.matrix(y), as.matrix(x), as.matrix(xtest), p_categorical = dcat,
            ...
            params$n_min, alpha = params$alpha, beta = params$beta, tau = params$tau, s = 1, kap = 1, burnin = params$burnin,
            mtry = params$mtry, draw_mu = TRUE,
            num_cutpoints = params$num_cutpoints, parallel = parl, random_seed = 10,no_split_penality=params$no_split_penality)
