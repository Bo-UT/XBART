...
get_XBART_params <- function(n, d, y) {
  XBART_params = list(num_trees = 30,                 # number of trees 
                      num_sweeps = 100,           # number of sweeps (samples of the forest)
...
fit = XBART(as.matrix(y), as.matrix(x), as.matrix(xtest), p_categorical = dcat,
            ...
            num_cutpoints = params$num_cutpoints, parallel = parl, random_seed = 200,no_split_penality=params$no_split_penality, burnin = params$burnin)
...
fhat.1 = apply(fit$yhats[, params$burnin:params$num_sweeps], 1, mean)
