...
Rcpp::List xbart_multinomial_predict_separate_trees(arma::mat X, double y_mean, size_t num_class, Rcpp::XPtr<std::vector<std::vector<std::vector<tree>>>> tree_pnt)
{
    ...
    // Predict
    model->predict_std_separate_trees(Xpointer, N, p, N_trees, N_sweeps, yhats_test_xinfo, *trees, output_vec);
